...
const POST_PROCESS_CLEAR: wgpu::Color = wgpu::Color {
    r: 0.0,
    g: 0.0,
    b: 0.0,
    a: 0.0,
...
pub struct WgpuPostProcessPlan {
    passes: Vec<WgpuPostProcessPassPlan>,
}
...
pub struct WgpuPostProcessChain {
    ...
    passes: Vec<WgpuPostProcessPipeline>,
    ping_pong: Vec<WgpuPostProcessTexture>,
}

struct WgpuPostProcessPipeline {
    pipeline: wgpu::RenderPipeline,
    params: PostProcessParams,
}

struct WgpuPostProcessTexture {
    width: u32,
    height: u32,
    format: wgpu::TextureFormat,
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Pod, Zeroable)]
struct WgpuPostProcessUniform {
    viewport: [f32; 4],
    params: [[f32; 4]; POST_PROCESS_PARAM_VEC4S],
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct WgpuPostProcessStep {
    source: WgpuPostProcessSource,
    target: WgpuPostProcessDestination,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum WgpuPostProcessSource {
    Scene,
    PingPong(usize),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum WgpuPostProcessDestination {
    PingPong(usize),
    Final,
}
...
fn aligned_uniform_stride_for<T>(device: &wgpu::Device) -> u64 {
    align_to(
...
fn wrapped_post_process_wgsl(user_source: &str) -> String {
    format!(
        r#"
struct PpUniform {{
    viewport: vec4<f32>,
    params: array<vec4<f32>, {POST_PROCESS_PARAM_VEC4S}>,
}};
...
@vertex
fn pp_vs(@builtin(vertex_index) vertex_index: u32) -> PpVertexOut {{
...
fn pp_sample(uv: vec2<f32>) -> vec4<f32> {{
    return textureSample(pp_input, pp_sampler, uv);
}}

fn pp_param(index: u32) -> vec4<f32> {{
...
@fragment
fn pp_fs(input: PpVertexOut) -> @location(0) vec4<f32> {{
...
fn post_process_steps(pass_count: usize) -> Vec<WgpuPostProcessStep> {
    (0..pass_count)
...
fn upload_post_process_uniforms(
    queue: &wgpu::Queue,
    buffer: &wgpu::Buffer,
    uniform_stride: u64,
    width: u32,
